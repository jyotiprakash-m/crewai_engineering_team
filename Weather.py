...
class Weather:
    ...
    This implementation uses the OpenWeatherMap current weather API. You must provide
    an API key when creating an instance.
    ...
    def __init__(self, api_key: str, base_url: Optional[str] = None, timeout: int = 10, cache_ttl: int = 300) -> None:
        ...
        # _cache maps (city, units) -> (timestamp, parsed_result)
    ...
    def make_api_request(self, endpoint: str, params: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if resp.status_code != 200:
            # OpenWeatherMap often returns a JSON with 'message' on error
    ...
    def parse_weather_data(self, data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if city == "":
            # If the API didn't return a name, leave as empty but still return
            city = city
    ...
    def get_weather_by_city(self, city_name: str, units: str = "metric") -> Dict[str, Any]:
        ...
        # Some APIs return error codes inside JSON; handle that
        if isinstance(raw, dict) and (str(raw.get("cod", "")) != "200"):
...
# If this module is run directly, do not perform network requests.
# Consumers should import Weather and use it in their own code.
